...
func insertCapecRecordsIntoDB(DBConn *sql.DB, capecRecords []defs.CapecRecord) error {
	insertPrefix := "INSERT IGNORE INTO enigma.capeclist (name, prerequisites, related_weakness, description) VALUES "
	...
	for i, record := range capecRecords {
		...
		buf.WriteString("('" + record.Name + "', '" + prerequisitesStr + "', '" + relatedWeaknessStr + "', '" + record.Description + "'), ")
